// this file will work if the babel setup properly, we can use es6 features here...
import gulp from "gulp";
import path from "path";
import child_process from "child_process";
import rimraf from "rimraf";
import webpack from "webpack";
import webpackConfig from "./webpack.config";
import webpackDevServer from "webpack-dev-server";

const $ = require("gulp-load-plugins")(); //we can use all gulp plugins without import/required

/*------------------------
Server*/

gulp.task("server:clean", cb => {
    rimraf("./build", () => cb());
});

gulp.task("server:compile", function(){
    return gulp.src("./src/server/**/*.js")
            .pipe($.changed("./build")) // changed is a plugin that only compile the modified js files into build
            .pipe($.sourcemaps.init())
            .pipe($.babel())
            .pipe($.sourcemaps.write(".", { sourceRoot:  path.join(__dirname, "src", "server")}))
            .pipe(gulp.dest("./build"));
});

gulp.task("server:build", gulp.series(
    "server:clean",
    "server:compile"
));

function watchServer(){
    return gulp.watch("./src/server/**/*.js", gulp.series("server:compile"))
        .on("error", () => {});
}

function runNodemon(){
    $.nodemon({
        script: "./server.js",
        watch: "build",
        ignore: ["**/__tests"],
        exec: "node --debug"
    });
}

function runServerTest(){
    $.nodemon({
        script: "./tests.js",
        watch: "build"
    });
}

function testServer(cb){
    child_process.exec("node ./tests.js", (err, stdout, stderr) => {
        console.log(stdout);
        console.error(stderr);
        if(err){
            cb(new $.util.PluginError("testServer", "test failed"));
        }else{
            cb();
        }
    });
}


gulp.task("server:test", 
    gulp.series(
        "server:build",
        testServer
    )
);

gulp.task("server:watch", 
    gulp.series(
        "server:build",
        watchServer
    )
);

gulp.task("server:dev", gulp.series(
    "server:build",
    gulp.parallel(
        watchServer,
        runNodemon
    )
));

gulp.task("server:test:dev", 
    gulp.series(
        "server:build",
        gulp.parallel(
            watchServer,
            runServerTest
        )
    )
);

/*------------------------
Client*/

const consoleStats = {
    colors: true,
    exclude: ["node_modules"],
    chunks: false,
    assets: false,
    timings: true,
    modules: false,
    hash: false,
    version: false
};

gulp.task("client:clean", cb => {
    rimraf("./public/build", () => cb());
});

gulp.task("client:build", gulp.series(
    "client:clean",
    buildClient
));


gulp.task("client:dev", gulp.series(
    "client:clean",
    runWebpackDevServer
));

function buildClient(cb){
    webpack(webpackConfig, (err, stats) => {
        if(err){
            return cb(err);
        }

        console.log(stats.toString(consoleStats));
        cb();
    });
}

function runWebpackDevServer(){
    const compiler = webpack(webpackConfig);
    const server = new webpackDevServer(compiler, {
        publicPath: "/build/",
        hot: true,
        stats: consoleStats
    });

    server.listen(8080, () => {});
}

// general tasks
gulp.task("dev", gulp.parallel(
    "server:dev",
    "client:dev"
));

gulp.task("build", gulp.parallel(
    "server:build",
    "client:build"
));